...
/**
 * Expose moduleRenderer initialization.
 */

module.exports = function(app){
  app.directive('moduleRenderer', moduleRenderer);
};
...
function moduleRenderer($compile) {
  ...
  function instantiateModule(config, scope) {
    ...
    var attrs = config.attributes = config.attributes || {};
    var template = createTemplate(config, attrs);
    return template(scope)[0];
  }

  function createTemplate(config, attrs){
    var template = '<div';
    ...
    scope.$watchCollection(attributes.moduleRenderer, function (boxConfigs) {
      ...
      boxConfigs.forEach(function (boxConfig, index) {
        ...
        if (existingIndex < 0) {
          ...
          // Remove from config
          visibleBoxConfigs.splice(existingIndex, 1);
